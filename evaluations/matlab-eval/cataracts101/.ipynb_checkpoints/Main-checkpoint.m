...
gt_files = dir(gt_root_folder);

for k = 1:length(gt_files)
    video_name = gt_files(k).name;
    if ~startsWith(video_name, ".")
        gt_path = [gt_root_folder video_name];
        phaseGroundTruths = [phaseGroundTruths gt_path];
    end
end
...
for i = 1:length(phaseGroundTruths)
    ...
    predFile = [predroot phaseGroundTruth(end-11 : end-4) '-pred.txt'];
    ...
    if(size(gt{1}, 1) ~= size(pred{1},1) || size(gt{2}, 1) ~= size(pred{2},1))
        error(['ERROR: [' phaseGroundTruth '] Ground truth and prediction have different sizes']);
    end
    
    if(~isempty(find(gt{1} ~= pred{1})))
        error(['ERROR: [' phaseGroundTruth '] The frame index in ground truth and prediction is not equal']);
    end
...
for iPhase = 1:length(phases)
    disp([sprintf('%25s', phases{iPhase}) '|' sprintf('%6.2f', meanJaccPerPhase(iPhase)) '|' sprintf('%6.2f', meanPrecPerPhase(iPhase)) '|' sprintf('%6.2f', meanRecPerPhase(iPhase)) '|']);
    disp('---------------------------------------------');
end
disp('================================================');
